...
vuint8m4_t __attribute__((always_inline)) inline branchless_table_lookup_opt(vuint8m4_t vec_indices, vint8m1_t offset_vec, size_t vl)
{
...
vuint32m4_t __attribute__((always_inline)) inline create_lookup_indices(vuint8m4_t data, size_t vl)
{
...
void base64_encode_rvv(uint8_t *input, uint8_t *output, size_t length)
{
    ...
    for (; length >= input_slice_e8m4; length -= input_slice_e8m4)
    {
        ...
        vuint32m4_t vec_lookup_indices = create_lookup_indices(vec_gather, vl);
