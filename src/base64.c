...
vuint8m1_t loadIndex()
{
    size_t vl = __riscv_vsetvl_e8m1(16);
    return __riscv_vle8_v_u8m1(gather_index_new, vl);
...
vuint8m1_t inline branchless_table_lookup_opt(vuint8m1_t vec_indices, vint8m1_t offset_vec, size_t vl)
{
...
static inline uint64_t rv_cycles(void)
{
    uint64_t cycle;
...
void __attribute__((noinline)) base64_encode(uint8_t *restrict input, uint8_t *output, size_t length)
{
    ...
    const vuint8m1_t vec_index = loadIndex();
    ...
    for (; length >= input_slice; length -= input_slice)
    {
        ...
        vuint8m1_t vec_gather = __riscv_vrgather_vv_u8m1(vec_input, vec_index, vl);
...
char *setupInputData()
{
    ...
    char *inputData = (char *)malloc(sizeof(char) * N); // 20 MB
...
int main(void)
{
    ...
    float speedup = ((double)timeElapsed_scalar / (double)timeElapsed_vector);
    printf("speedup %.02f %%\n", speedup*100);
