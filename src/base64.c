...
vuint8m4_t loadIndex()
{
    ...
    return __riscv_vle8_v_u8m4(gather_index_lmul4, vl);
...
vuint8m4_t __attribute__((always_inline)) inline branchless_table_lookup_opt(vuint8m4_t vec_indices, vint8m4_t offset_vec, size_t vl)
{
    vuint8m4_t result = __riscv_vssubu_vx_u8m4(vec_indices, 51, vl);

    vbool2_t ge_26_mask = __riscv_vmsgtu_vx_u8m4_b2(vec_indices, 25, vl);
    vbool2_t le_51_mask = __riscv_vmsleu_vx_u8m4_b2(vec_indices, 51, vl);

    vbool2_t mask = __riscv_vmand_mm_b2(ge_26_mask, le_51_mask, vl);

    result = __riscv_vmerge_vxm_u8m4(result, 13, mask, vl);

    offset_vec = __riscv_vrgather_vv_i8m4(offset_vec, result, vl);

    vint8m4_t ascii_vec = __riscv_vadd_vv_i8m4(__riscv_vreinterpret_v_u8m4_i8m4(vec_indices), offset_vec, vl);
...
vuint32m4_t create_lookup_indices_opt(vuint8m4_t data, size_t vl)
{

    vuint32m4_t const_vec_ac = __riscv_vmv_v_x_u32m4(0x04000040, vl);
    vuint32m4_t const_vec_bd = __riscv_vmv_v_x_u32m4(0x01000010, vl);
...
void __attribute__((noinline)) base64_encode(uint8_t *restrict input, uint8_t *output, size_t length)
{
    ...
    const vuint8m4_t vec_index = loadIndex();

    vint8m4_t offset_vec = __riscv_vle8_v_i8m4(offsets, vlmax_8);
    ...
    for (; length >= input_slice; length -= input_slice)
    {
        ...
        vl = __riscv_vsetvl_e8m4(input_slice);

        vuint8m4_t vec_input = __riscv_vle8_v_u8m4(input, vl);

        vl = __riscv_vsetvl_e8m4(vlmax_8);

        // TODO use vrgatherei16.vv here when possible
        // uint64_t begin = rv_cycles();
        vuint8m4_t vec_gather = __riscv_vrgather_vv_u8m4(vec_input, vec_index, vl);
        ...
        vuint32m4_t vec_lookup_indices = create_lookup_indices_opt(vec_gather, vl);
        ...
        vuint8m4_t base64_chars = branchless_table_lookup_opt(__riscv_vreinterpret_v_u32m4_u8m4(vec_lookup_indices), offset_vec, vl);
